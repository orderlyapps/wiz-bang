...
export function AvOverseerContent({
  ...
  const congregation_permissions = permissions.filter(
    (ap) => ap.congregation_id === congregation_id,
  );

  const permitted_publishers = Object.values(
    congregation_permissions
      .map((ap) => {
        const publisher = publishers.find((p) => p.auth_id === ap.auth_user_id);
  ...
  const handleDelete = (permission_id: string) => {
    avOverseerPermissionCollection.delete(permission_id);
  };
    ...
    <>
      ...
      <IonList inset>
        {permitted_publishers.length === 0 ? (
          <IonItem>
            <IonLabel>No publishers with AV overseer permission.</IonLabel>
          </IonItem>
        ) : (
