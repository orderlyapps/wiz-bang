...
export function AddPublisherModal({ is_open, on_dismiss }: AddPublisherModalProps) {
  ...
  const permitted_auth_ids = new Set(
    (permissions ?? [])
      .filter((ap) => !congregation_id || ap.congregation_id === congregation_id)
      .map((ap) => ap.auth_user_id),
  );
  ...
  const handleAdd = (publisher: Publisher) => {
    ...
    avOverseerPermissionCollection.insert({
      ...
      <IonContent>
        <IonList inset>
          {available_publishers.length === 0 ? (
            <IonItem>
              <IonLabel>All publishers already have AV overseer permission.</IonLabel>
            </IonItem>
          ) : (
