...
export function AddPublisherModal({ is_open, on_dismiss }: AddPublisherModalProps) {
  ...
  const permitted_auth_ids = new Set(
    (permissions ?? [])
      .filter((sp) => !congregation_id || sp.congregation_id === congregation_id)
      .map((sp) => sp.auth_user_id),
  );
  ...
  const handleAdd = (publisher: Publisher) => {
    ...
    serviceOverseerPermissionCollection.insert({
      ...
      <IonContent>
        <IonList inset>
          {available_publishers.length === 0 ? (
            <IonItem>
              <IonLabel>All publishers already have service overseer permission.</IonLabel>
            </IonItem>
          ) : (
